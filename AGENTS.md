...
- **ИЕРАРХИЯ ИСТОЧНИКОВ ПРИ ПОРТИРОВАНИИ:** `js_reference` > Поведение клиента > `server_reference/` (C# референс) > Rust-сервер. Если C# референс расходится с `js_reference`, `js_reference` является эталоном.
...
- **НЕ КОММИТИТЬ СЛУЖЕБНЫЙ ИСХОДНЫЙ КОД:** Запрещено добавлять в коммиты каталоги `client/` и `server_reference/`.
