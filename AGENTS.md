...
- **ЕДИНАЯ ВЕТКА** — работать только в локальном `main` и `origin/main`. Не
  создавать новые git-ветки, worktree, fork-ветки или временные branch-based
  потоки без прямой явной просьбы пользователя. Запрещены `git switch -c`,
  `git checkout -b`, `git branch <name>` и аналоги. Если внешний инструмент уже
  создал лишние ветки — не использовать их; после завершения всех правок и
  отдельного подтверждения пользователя удалить все локальные и remote-ветки,
  кроме локального `main`, `origin/main` и служебного `origin/HEAD`.
