...
use crate::game::player::{PlayerFlags, PlayerMetadata, PlayerPosition, PlayerStats, PlayerUI};
...
pub async fn handle_pack_action(
    ...
    match pack.pack_type {
        PackType::Resp => {
            ...
            let Ok(cells) = pack.pack_type.building_cells() else {
                tracing::error!(pack_type = ?pack.pack_type, "Missing building config for pack action");
                return;
            };
            if !cells
                .iter()
