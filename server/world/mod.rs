...
impl Layer {
    pub fn open(
        ...
        let total_bytes = u64::from(chunks_w) * u64::from(chunks_h) * cells_per_chunk * (data_type.size() as u64);
...
impl WorldProvider for World {
    #[inline] fn name(&self) -> &str { &self.name }
    #[inline] fn chunks_w(&self) -> u32 { self.chunks_w }
    #[inline] fn chunks_h(&self) -> u32 { self.chunks_h }
    #[inline] fn cell_defs(&self) -> Arc<CellDefs> { self.cell_defs.clone() }
    #[inline] fn cells_width(&self) -> u32 { self.chunks_w * CHUNK_SIZE }
    #[inline] fn cells_height(&self) -> u32 { self.chunks_h * CHUNK_SIZE }

    #[inline]
    fn valid_coord(&self, x: i32, y: i32) -> bool {
        x >= 0 && y >= 0 && x.cast_unsigned() < self.cells_width() && y.cast_unsigned() < self.cells_height()
    }

    fn get_cell(&self, x: i32, y: i32) -> u8 {
        if !self.valid_coord(x, y) { return 0; }
    ...
    fn set_cell(&self, x: i32, y: i32, cell: u8) {
        if !self.valid_coord(x, y) { return; }
        let (ux, uy) = (x.cast_unsigned(), y.cast_unsigned());
    ...
    fn get_durability(&self, x: i32, y: i32) -> f32 {
        if !self.valid_coord(x, y) { return 0.0; }
    ...
    fn read_chunk_cells(&self, chunk_x: u32, chunk_y: u32) -> Vec<u8> {
        ...
        if chunk_x >= self.chunks_w || chunk_y >= self.chunks_h { return vec![0u8; n]; }
...
impl World {
    pub fn new(name: &str, chunks_w: u32, chunks_h: u32, cell_defs: CellDefs, state_dir: &Path) -> Result<Self> {
        let mut layers = HashMap::new();
        let cells_path = state_dir.join(format!("{name}.mapb"));
        let is_new = !cells_path.exists();

        layers.insert("cells".to_string(), RwLock::new(Layer::open(cells_path, chunks_w, chunks_h, LayerType::U8)?));
        layers.insert("road".to_string(), RwLock::new(Layer::open(state_dir.join(format!("{name}_road.mapb")), chunks_w, chunks_h, LayerType::U8)?));
        layers.insert("durability".to_string(), RwLock::new(Layer::open(state_dir.join(format!("{name}_durability.mapb")), chunks_w, chunks_h, LayerType::F32)?));
        ...
        if is_new {
            ...
            world.layers.get("road").unwrap().write().mmap.fill(cell_type::EMPTY);
    ...
    pub fn chunk_pos(x: i32, y: i32) -> (u32, u32) {
        (x.max(0).cast_unsigned() / CHUNK_SIZE, y.max(0).cast_unsigned() / CHUNK_SIZE)
    }
}
