...
pub fn send_chat_login_per_reference(state: &Arc<GameState>, tx: &mpsc::UnboundedSender<Vec<u8>>, pid: PlayerId) {
    ...
    if !msgs_snapshot.is_empty() {
        // Референс передаёт тег "FED" в конструктор даже при другом currentchat — оставляем фактический `tag`.
        send_u_packet(tx, "mU", &chat_messages(&tag, &msgs_snapshot).1);
    }
}
