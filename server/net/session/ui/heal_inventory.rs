    ...
    for ddx in -1..=1 {
        for ddy in -1..=1 {
            ...
            if state.world.valid_coord(tx, ty) {
                state.world.set_cell(tx, ty, cell_type::EMPTY);
...
pub fn use_razryadka(state: &Arc<GameState>, pid: PlayerId) -> bool {
    let pos = state
        .query_player(pid, |ecs: &bevy_ecs::prelude::World, entity| {
            ecs.get::<PlayerPosition>(entity).map(|p| (p.x, p.y))
        })
        .flatten();
    let Some((px, py)) = pos else {
        return false;
...
pub fn use_c190(state: &Arc<GameState>, pid: PlayerId) -> bool {
    ...
    for i in 1..=10 {
        ...
        for entry in &state.active_players {
            let opid = *entry.key();
            let h = state
                .query_player(opid, |ecs: &bevy_ecs::prelude::World, entity| {
                    let p = ecs.get::<PlayerPosition>(entity)?;
        ...
        if let Some(t_pid) = hit {
            let tx_death = state
                .modify_player(t_pid, |ecs: &mut bevy_ecs::prelude::World, entity| {
