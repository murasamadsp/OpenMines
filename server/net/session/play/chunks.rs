...
pub fn check_chunk_changed(state: &Arc<GameState>, tx: &mpsc::UnboundedSender<Vec<u8>>, pid: PlayerId) {
    let p_data = state.query_player(pid, |ecs, entity| {
        ...
        Some((pos.chunk_x(), pos.chunk_y(), view.last_chunk, view.visible_chunks.clone()))
    }).flatten();

    let Some((cx, cy, last_chunk, old_visible)) = p_data else { return; };
    if Some((cx, cy)) == last_chunk { return; }
    ...
    let flush_sub_packets = |sub_packets: &mut Vec<Vec<u8>>, sent_batches: &mut usize, sub_batch_bytes: &mut usize, tx: &mpsc::UnboundedSender<Vec<u8>>, _pid: PlayerId| {
        if sub_packets.is_empty() { return; }
        let bundle = hb_bundle(sub_packets.as_slice()).1;
        send_b_packet(tx, "HB", &bundle);
        *sent_batches += 1; sub_packets.clear(); *sub_batch_bytes = 0;
    };

    for (ncx, ncy) in new_visible.iter().copied() {
        if old_visible_set.contains(&(ncx, ncy)) { continue; }
        ...
        for entry in &state.active_players {
            ...
            let bot_data = state.query_player(opid, |ecs, entity| {
                ...
                if p.chunk_x() == ncx && p.chunk_y() == ncy {
                    Some(hb_bot(net_u16_nonneg(opid), net_u16_nonneg(p.x), net_u16_nonneg(p.y), net_u8_clamped(p.dir, 3), net_u8_clamped(s.skin, 255), net_u16_nonneg(s.clan_id.unwrap_or(0)), 0))
                } else { None }
            }).flatten();
        ...
        if sub_packets.len() >= CHUNK_BUNDLE_MAX_SUBPACKETS || sub_batch_bytes >= CHUNK_BUNDLE_MAX_BYTES {
            flush_sub_packets(&mut sub_packets, &mut sent_batches, &mut sub_batch_bytes, tx, pid);
        }
    }

    for (ocx, ocy) in old_visible {
        if new_visible.contains(&(ocx, ocy)) { continue; }
    ...
    state.modify_player(pid, |ecs, entity| {
        ...
        if (ocx, ocy) != (ncx, ncy) {
            state.chunk_players.get_mut(&(ocx, ocy)).map(|mut e| e.retain(|&id| id != pid));
