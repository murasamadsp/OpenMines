...
pub fn handle_pack_action(state: &Arc<GameState>, tx: &mpsc::UnboundedSender<Vec<u8>>, pid: PlayerId, x: i32, y: i32) {
    ...
    match pack.pack_type {
        PackType::Resp => {
            ...
            send_u_packet(tx, "OK", &ok_message("Респ", "Точка возрождения установлена").1);
        }
        _ => crate::net::session::ui::gui_buttons::handle_gui_button(state, tx, pid, &format!("pack_op:open:{}:{}", x, y)),
    }
}
