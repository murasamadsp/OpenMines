        ...
        loop {
            ...
            for (pid, player_data) in &dirty_rows {
                if let Err(e) = state.db.save_player(player_data) {
                    ...
                    // dirty remains true; next cycle will retry
                } else {
                    // Save succeeded — now clear dirty so we don't re-save unchanged data.
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            for entity in dirty_entities {
                let row = state
                    .modify_building(entity, |ecs, ent| {
