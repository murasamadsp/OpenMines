...
- неограниченные или неразделённые очереди превращают burst в latency и рост памяти;
...
```text
TCP sessions -> SessionHub -> QueuedGameCommand -> Simulation
                    ^                       |
                    |------ GameEvent ------|
                                            |
...
| 1. Session/output owner | существенно начат | common authenticated envelope и отсутствие sender capability |
...
| 6. Active/due work | programmator/guns/standing-cell hazards закрыты | zero scan-all для alive/granular sleeping actors |
...
Periodic player/building dirty flush теперь используют entity registries с
работой `O(dirty)`; player registry проверяет generation при reconnect. Дешёвый
`1 idle player` после
этого требует due-only hazards/guns/programmator вместо periodic entity scans.

Event-driven wait теперь сочетается с bounded QoS ingress и graceful drain
accepted `DueAction`. Оставшиеся gates этого слоя - перенести прямые durable
bypass и довести active/due registries до sleeping actors; их порядок находится
в `SERVER_MIGRATION_STATUS.md`.

Этап 3 не завершён. Прямые persistence bypass ещё есть у program операций
(open/rename/delete/copy), GUI/auction и shutdown
...
- в ECS и `GameCommand` нет sender types;
...
- bounded typed ingress закрыт; сохранять lifecycle/gameplay/internal QoS policy;
...
- hazards: position-transition activation и damage deadlines;
