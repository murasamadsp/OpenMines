...
**Только graceful drain accepted DueAction.** Текущий дефект:
...
Protector/Raz дополнительно возвращают `building_removals`, которые сейчас
отправляются обратно во внешний command channel. После закрытия ingress этот
follow-up теряется.
...
- файл `crates/openmines-server/src/tasks/simulation.rs`, test filters:
  `tasks::simulation::shutdown_tests::future_due_action_runs_at_real_deadline_before_shutdown_finishes`,
  `tasks::simulation::shutdown_tests::quiescing_drains_buffered_command_that_admits_due_action`,
  `tasks::simulation::shutdown_tests::protector_followup_survives_closed_external_ingress`;
- файл `crates/openmines-server/src/tasks/simulation/tick/tests.rs`, filter:
  `tasks::simulation::tick::tests::internal_building_delete_saturation_preserves_head_and_runtime_state`;
- integration с реальным `PersistenceRuntime` заканчивается согласованными
  DB/ECS/world, filter:
  `tasks::simulation::shutdown_tests::shutdown_waits_for_internal_delete_completion`;
- после quiescing пусты due queue, internal delete FIFO, death/box backlogs и
  persistence backlog;
- существующие exact-deadline/order tests Boom/Protector/Raz остаются зелёными.
