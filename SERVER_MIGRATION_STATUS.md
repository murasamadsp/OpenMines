...
1. Прочитать разделы `Что горит`, `Текущий кодовый срез` и `Запрещённые
   решения` ниже.
2. Текущий срез - перевести `channel_chat` на typed command/apply/effects и
   bounded persistence, не меняя legacy wire. Не смешивать его с ECS ownership
   или multicore.
...
**Следующий vertical slice: channel chat.** Ручной trace показал `201.75ms`
CPU-bound command. Перевести channel chat на typed command/apply/effects и
bounded persistence/fanout, не маскируя проблему локальным micro-optimization.
