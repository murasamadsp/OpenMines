...
- **КОММИТИТЬ ВСЁ.** Перед коммитом — `git status`. Если есть untracked или modified файлы относящиеся к задаче (включая `.claude/`, конфиги, документацию) — включить в тот же коммит. Не оставлять ничего незакоммиченным после завершения задачи.
...
| Чат | `social/chat.rs` + `social/commands.rs` | Глобальный + клановый + локальный, DB, admin cmds |
...
| Смерть/Респавн | `play/death.rs` | Crystal box, TP, HP reset, hurt_player_pure |
