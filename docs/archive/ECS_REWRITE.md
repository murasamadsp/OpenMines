...
- **Формат хранения** (`.map` + sqlite) неизменен — roundtrip-тесты.
...
- [~] P3 — Tickrate-бакеты + interest-replication: по сути УЖЕ реализовано.
  Tickrate-бакеты = системы самотроттлятся (sand 100мс/alive 5с/acid 3с/building
  1с/1ч, early-return дёшев). Interest-replication = `broadcast_to_nearby` по 5×5
  чанков через `chunk_players` (без глобального фан-аута). Слепо лепить run_if =
  повтор P1. Делать только при измеренной нужде.
- [ ] P4 — Шардинг (несколько sim-тасков по зонам = распараллелить симуляцию по
  ядрам). ЮЗЕР: «параллелить симуляцию НУЖНО, но в самое далёкое будущее».

## ЗАМЕР (loadtest, single-process, локально; loadtest co-located → пессимистично)
Тул: `cargo run --release --bin loadtest -- --clients N` (`tools/loadtest`).
Пред-засев игроков в БД + Regular-auth (заход через регистрацию = auth-failure,
банит IP после 6). Замер делает СЕРВЕР через tickprof (`M3R_LOG=warn`).

Результаты (мир 8×8 чанков, спавн кластерно → worst-case фан-аут):
- **1000 игроков**: 9 OVER-BUDGET тиков / 20с. Стабильно.
- **3000**: 45 OVER-BUDGET + 1 SLOW (>50мс). Ещё держится, 0 обрывов.
- **6000**: обрывы пошли (449); `schedule.run` подскочил 25→61→**128мс**, 10мс-тик
  отстал → клиенты посыпались.
ВЫВОД (однозначный, неизменный на всех N): **`dispatch` тривиален** (200–680µs
даже на 340 действий/тик — сеть/движение масштабируются за счёт tokio multi-core +
sync hot-path). **Потолок = одноядерный `schedule.run` (физика)**. Колено ~3–4k
игроков на этой машине до того как тики физики стабильно пробивают бюджет.
Рычаги на 10k: P4 шардинг (физику по ядрам) + оптимизация sand (см. ниже).

## Параллелизм сервера (зафиксировано)
- Сеть/I-O: ПАРАЛЛЕЛЬНО на всех ядрах (`#[tokio::main]` = multi-thread work-steal).
- Симуляция (тик): ОДНОЯДЕРНАЯ (`schedule.run` под одним `ecs.write()`). Потолок.
- Генерация мира: параллельно (rayon `par_chunks`), но разово.

## Смежные задачи (всплыли по ходу — верифицировать отдельно)
- **САЙПУЧКА (`sand_physics_system`) — будущий CPU-хог.** ЮЗЕР: «будет сжирать
  99% ЦПУ. НАДО». Замер подтвердил: спайки `schedule` — это тик'и где fired sand
  (каждые 100мс); на 6k игроков один проход = 60–128мс. Оптимизировать:
  активные-клетки/дельты вместо полного скана, бакетизация, возможно вынос в свой
  тред/rayon. Связано с P4.
- **Генерация мира медленная/возможно криво портирована** (2048² регенится
  >50с на старте; юзер: «плохо портирована с оригинала»). НО: мир персистнут в
  mmap, regen разовый; прод-мир уже существует → фикс генератора near-moot для
  живой игры (применить = регенерировать = снести мир, урок 4). Низкий приоритет.
...
- **Генерация мира медленная/возможно криво портирована** (2048² регенится
  ...
  `World.cs`/генератором (`crates/openmines-shared/src/world/generator.rs`, `anl.rs`, `sectors_gen.rs`)
