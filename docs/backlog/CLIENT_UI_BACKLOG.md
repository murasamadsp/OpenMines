...
- Доказательство: серверный builder уже фиксирует старые проблемы, например
  ...
  (`crates/server/openmines-server/src/net/session/ui/horb.rs`).
- Что делать: запретить ручную сборку HORB JSON в новых/старых хендлерах. Всё
  переводить на typed builder `crates/server/openmines-server/src/net/session/ui/horb.rs`, добавить
...
- Static gate: `python3 tools/ui_layout_audit.py --matrix-only --horb-risk`.
  ...
  emit в `crates/server/openmines-server/src/net/session/ui/horb.rs`. Любой рост этого счётчика означает
...
### UI-007. `UpdateProgramm(#p)` проходит через editor path

- Симптом: `#p` на старте/логине может моргнуть или снова поднять editor path.
