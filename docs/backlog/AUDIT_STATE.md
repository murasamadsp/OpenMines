...
Не поддерживается в этом историческом файле. Следовать разделам `Что горит` и
`Текущий кодовый срез` в `SERVER_MIGRATION_STATUS.md`. Любую намеренную девиацию от C#/JS reference
