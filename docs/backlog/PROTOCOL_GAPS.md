...
- текущий Rust dispatcher: `crates/server/openmines-server/src/net/session/dispatch/ty.rs`;
...
1. Programmator packets: keep `Pope`/`PROG`/`pRST`/`#P`/`#p` state-machine as
   ...
   fallback are covered by Rust tests in `crates/server/openmines-server/src/game/actors/programmator.rs`.
