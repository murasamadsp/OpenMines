...
| `PROG` | payload валиден | сохранить source, выбрать программу, попытаться запустить | `Gu`, затем `#p {id,title,source}`, затем `@P 0/1`; при `@P 0` дополнительно `OK` |
| `PROG` | payload битый/truncated | не сохранять и остановить старый running state | `@P 0`, затем `OK` |
| `pRST` | программа запущена | остановить running state | `Gu`, затем `@P 0` |
