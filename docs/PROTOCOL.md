...
Цель этого контракта — держать Unity UI в предсказуемом состоянии. Клиент
различает `#P` и `#p`: `#P` открывает/держит редактор открытым, `#p`
обновляет выбранную программу и сбрасывает активный editor view.
...
| `PROG` | payload валиден, но программа не запущена | сохранить source, выбрать программу, вернуть ошибку запуска | `Gu`, затем `#p {id,title,source}`, `@P 0`, `BH 0`, затем `OK` |
...
- Сервер не должен отправлять ложный `@P 1`, если parse/run программы не
  удался.
- `#p` обязателен последним после успешного running `PROG`: Unity `@P 1`
  вызывает `ChangeProgTo(true)` и показывает объект редактора, а
  `UpdateProgramm()` затем гидратит программу и снова скрывает editor view.
