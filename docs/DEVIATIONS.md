...
| Где | Девиация | Причина |
|-----|----------|---------|
| `crates/openmines-server/src/tasks/simulation.rs` `spawn_game_tick_loop` | authoritative owner при panic завершает процесс с кодом `101`; потенциально повреждённый ECS не продолжает работу и не respawn'ится внутри процесса | **[SAFETY]** fail-stop вместо продолжения с неизвестными invariants |
