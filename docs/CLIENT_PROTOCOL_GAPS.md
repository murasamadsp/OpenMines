# Расхождения клиент ↔ референс (что есть в клиенте, но не/неверно в `server_reference/`)

> **Зачем этот документ.** `server_reference/` — НЕПОЛНАЯ реконструкция
...
>
> Прошлый аудит (`AUDIT.md`/`ROADMAP.md`) ставил ✅ сверяясь с неполным
...
> конфиг сцены; **клиент неизменяем** → серверного фикса нет. Решение
> юзера: **забить** (косметика; «пишем сервер»). Тупиковые версии в
...
> - ✅ **Дубли на реконнекте**: login слал полную историю `mU`, клиент
>   `muHandler` `AddLine`'ит всё при `ch==currentChat` (визуал НЕ
>   id-дедупится). ИСПРАВЛЕНО: login=mO-only, история через `Chin`-
>   resync по `getLasts()` (§2). Probe-verified (реконнект с
>   актуальным lastid → `mU h=[]`, дублей нет).
...
**Почему полная пере-отправка ломается:** клиент `muHandler` вызывает
...
реконнекте login слал полную историю `mU` → `muHandler` `AddLine`'ил
...
- `send_chat_login_per_reference` (`init_player`) шлёт **только `mO`**
  (восстановить `currentChat` UI + триггер `moHandler` clear+re-render
  существующей History на реконнекте). Историю login НЕ шлёт.
...
## 7. Прочие события клиента БЕЗ обработки на сервере
...
### 7.1 `THID` — прогресс/скрытие туториала — ❌ (fire-and-forget)
...
### 7.2 `PCOP <programId>` — копия программы программатора — ❌ (доку+сам)
...
Реализуемо отдельным проходом.

### 7.3 `GDon <method>` / `Help "_"` — донат / помощь — ❓ нужно решение
...
### 7.4 `Miso "0"` — миссии — ⚠ НЕ §7-дыра: подсистема ЕСТЬ в референсе
...
(сейчас dispatch → пустой no-op!), `Miso`, `Rndm`, `TAGR`/`TAUR`.
Отдельная крупная задача 1:1-порта `Sys_Miss` (НЕ реверс по клиенту).

### Сводка

| Событие | Триггер клиента (file:line) | Payload | Класс | Статус |
|---|---|---|---|---|
| `THID` | TutorialNavigation.cs:132/141 | marker/"TIMEOVER" | fire-and-forget | ❌ |
| `PCOP` | ProgrammatorManager.cs:106 | programId | доку+сам | ❌ |
| `GDon` | GUIManager.cs:162 | METHOD | решение юзера | ❓ |
| `Help` | GUIManager.cs:117 | "_" | решение юзера | ❓ |
| `Miso` | MissionPad.cs:12 | "0" | **1:1 порт Sys_Miss** | ❌ |
...
- 2026-05-17: создан. Зафиксированы §1 (mU id, ✅), §2 (Chin no-op, ✅),
  §3–6 (Cmen/Choo/Cset/Cpri — спроектированы по клиенту), §7 (прочие
  дыры). Источник правды — `client/`, т.к. `server_reference/` для чата
  неполон (`/////FIX THIS SH`).
- 2026-05-17: §3–6 РЕАЛИЗОВАНЫ и верифицированы прямым protocol-пробом
  ...
  Прочее (§7: GDon/Help/Miso/PCOP/THID) — по-прежнему ❌.
- 2026-05-17 (репорты юзера с real-client, исправлено+probe-verified):
  ...
  - **Дубли на реконнекте**: §2 переписан — `Chin` НЕ no-op, а
    ...
    Итог: login=`mO`-only, история через `Chin` (`"_"`→полная,
    `"1:cur:lasts"`→`id>lastid`). Снято: login слал полную историю
    → клиент `muHandler` `AddLine`'ил всё (визуал не id-дедупится)
    → дубли. Probe: реконнект с актуальным lastid → `mU h=[]`.
  - **«старые мелкие» диагностирован**: НЕТ пакета размера —
    `ChatLineInfo.cs:54` `if(gid<=0) fontSize=10`. Тот же корень что
    «цвета/кэш»: история шлёт `user_id=0`/`color`-рассинхрон/`time`
    sec≠min. → §1 pass-2, ОТКРЫТО (схема `chat_messages.player_id`
    +`color`, `time` минуты везде). НЕ сделано — следующая сессия.
