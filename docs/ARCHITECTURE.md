...
Graceful shutdown пока имеет отдельную correctness-дыру: future DueAction может
быть уже принят после списания item, но уничтожен при выходе owner loop.
Protector/Raz building-removal follow-up также возвращается во внешний command
channel. Следующий срез переводит shutdown в `Quiescing` с real-deadline drain,
owner-local delete FIFO и persistence completion barrier.
