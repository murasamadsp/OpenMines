...
prog.login.selected | Player.Init | server push | selected_program from DB | player/init.rs::init_player | #p with @P/BH in init stream | не открывать #P поверх игры
```
...
admin.open | admin gear | ADMN | empty | dispatch/ty.rs ADMN branch | handler-specific | current window admin action
```
