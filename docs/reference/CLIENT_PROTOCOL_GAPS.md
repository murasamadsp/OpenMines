...
> Unity-клиента. Сетевой wire legacy-клиента не меняется; исходный код клиента
> может рефакториться только по явному запросу. Единая behavior hierarchy:
> `js_reference` > наблюдаемое поведение клиента > C#-референс > Rust-сервер.
> `docs/PROTOCOL.md` нормализует подтверждённый контракт. Здесь подробно
> зафиксировано КАЖДОЕ такое место: что шлёт/ждёт
...
> каждой фичи по общей evidence hierarchy, а не только против одного референса.
>
...
> (подтверждённый client evidence) реализовано:
...
против `match`-веток `crates/server/openmines-server/src/net/session/dispatch/ty.rs`.)
