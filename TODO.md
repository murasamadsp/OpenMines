...
- `/skill`: команда в игре "не работает" по ручной проверке. Сначала проверить:
  права отправителя (`role=2`), wire/DB-код скилла (`U` для геологии, не `GEO`),
  online-target `me`, ECS-мутирование, sync-пакеты `@S/LV/sp/@L/@B`,
  персист в `players.skills`.
- Скиллы: многие скиллы не подключены к действиям, не накапливают опыт или
  ...
  -> exp hook -> UI/packet sync.
- Программатор: при запуске программы снова открывается GUI редактора. Это
  регрессия GUI/wire сценария: старт программы не должен показывать редактор.
