...
/// mC — код цвета поля ввода чата (short `0..=19`).
///
/// Клиент `ChatManager.cs muHandler`→`mcHandler`: `short.Parse(msg)` →
/// `Color.HSVToRGB(num/20, ...)`. Реф: `Chat/ChatColorPacket.cs` (валидный
/// диапазон `[0,20)`). Нет в `server_reference` логики — см.
/// `docs/CLIENT_PROTOCOL_GAPS.md` секция 5.
#[must_use]
...
/// [1B tag 'D'] [1B fx] [u16 LE dir] [1B color] [u16 LE x] [u16 LE y] [u16 LE `bot_id`]
#[must_use]
...
#[must_use]
pub fn hb_crystal_mine_fx(bot_id: u16, x: u16, y: u16, amount: u16, color: u8) -> Vec<u8> {
    hb_directed_fx(bot_id, x, y, 2, amount, color)
}
