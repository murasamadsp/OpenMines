...
use crate::packets::*;
use crate::{Packet, b_packet, u_packet};
use bytes::BytesMut;
...
#[test]
fn bibika_is_bb_with_underscore_payload() {
    let (event, payload) = bibika();
    assert_eq!(event, "BB");
    assert_eq!(payload, b"_");
}
...
#[test]
fn money_clamps_negative_values_like_reference_send_money() {
    let (_event, payload) = money(-1, -50);
    assert_eq!(payload, br#"{"money":0,"creds":0}"#);
}
...
#[test]
fn inventory_show_formats_all_selected_grid() {
    let (event, payload) = inventory_show(&[(1, 5), (2, 3)], 2, 9);
    assert_eq!(event, "IN");
    assert_eq!(payload, b"show:9:2:1#5#2#3");
}
...
#[test]
fn mission_packets_match_client_handlers() {
    let (event, payload) = mission_panel("img.png", 200, 100, 37, "Mine %P%");
    assert_eq!(event, "MM");
    assert_eq!(payload, b"img.png#200#100#37#Mine %P%");

    let (event, payload) = mission_progress(7, 10);
    assert_eq!(event, "MP");
    assert_eq!(payload, b"7:10");

    let (event, payload) = mission_notification("Open chat", 12, 34, 2, "_CHATMENU");
    assert_eq!(event, "MN");
    assert_eq!(payload, b"Open chat#12#34#2#_CHATMENU");
}
...
fn hb_directed_fx_layout_is_fx_dir_color_x_y_botid() {
    ...
    assert_eq!(bytes, vec![b'D', 4, 5, 0, 6, 2, 0, 3, 0, 1, 0]);
}

#[test]
fn semantic_directed_fx_builders_keep_wire_layout() {
    assert_eq!(hb_dig_fx(9, 2, 3, 1), hb_directed_fx(9, 2, 3, 0, 1, 0));
    assert_eq!(
        hb_world_blast_fx(2, 3, 9, 2),
