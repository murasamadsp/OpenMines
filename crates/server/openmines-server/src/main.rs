pub use openmines_common::config;
pub use openmines_common::env_config;
pub use openmines_common::logging;
pub use openmines_common::metrics;
pub use openmines_common::time;
...
pub(crate) fn test_config_path(relative: &str) -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
        .expect("server crate must live inside crates/server/ folder under workspace root")
        .join(relative)
}
