    ...
    match completion {
        ...
        crate::game::PersistenceCompletion::ProgramOpened { request, result } => {
            ...
            match result {
                ...
                crate::game::ProgramOpenResult::Rejected => programmator_error_effect(
                    request.session_id,
                    request.player_id,
        ...
        crate::game::PersistenceCompletion::ProgramRenamed { request, result } => {
            ...
            match result {
                ...
                crate::game::ProgramRenameResult::Rejected => programmator_error_effect(
                    request.session_id,
                    request.player_id,
                    "Программа недоступна.",
                ),
        ...
        crate::game::PersistenceCompletion::ChatMenuLoaded { request, result } => {
            ...
            match result {
                crate::game::ChatMenuResult::Success { mut channels } => {
                    ...
                    let batch = crate::net::session::wire::PacketBatch::default();
                    crate::net::session::wire::send_u_packet(&batch, "mL", &ml.1);
                    crate::net::session::wire::send_u_packet(&batch, "mN", &mn.1);
                    CommandEffects {
                        events: vec![crate::game::GameEvent::SessionBatch {
                            session_id: request.session_id,
                            player_id: request.player_id,
...
fn programmator_error_effect(
    session_id: crate::game::SessionId,
    player_id: crate::game::PlayerId,
    message: &str,
) -> CommandEffects {
    let batch = crate::net::session::wire::PacketBatch::default();
    crate::game::logic::gui::programmator_gui::send_programmator_action_error(&batch, message);
    CommandEffects {
        events: vec![crate::game::GameEvent::SessionBatch {
            session_id,
            player_id,
