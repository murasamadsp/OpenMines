...
#[cfg(test)]
use crate::protocol::packets::{money, tp};
#[cfg(test)]
    ...
    if let Some(ref window) = handled {
        ...
        if let Some(rest) = window.strip_prefix("market:") {
            ...
            if parts.len() >= 2
                && let (Ok(x), Ok(y)) = (parts[0].parse::<i32>(), parts[1].parse::<i32>())
            {
                crate::game::logic::gui::market_gui::open_market_admin_gui(state, tx, pid, x, y);
                return;
            }
        }
        ...
        if let Some(rest) = window.strip_prefix("pack:") {
            ...
            if parts.len() == 2
                && let (Ok(x), Ok(y)) = (parts[0].parse::<i32>(), parts[1].parse::<i32>())
            {
                crate::game::logic::gui::pack_gui::open_pack_admin_gui(state, tx, pid, x, y);
                return;
            }
        }
    }
        ...
        let money_result = state
            .modify_player(target_pid, |ecs: &mut bevy_ecs::prelude::World, entity| {
                    ...
                    || ecs.get::<PlayerFlags>(entity).is_none()
                {
                    ...
                    if let Some(conn_tx) = conn_tx_clone {
