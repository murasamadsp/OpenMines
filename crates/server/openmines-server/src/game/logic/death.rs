    ...
    let (rx, ry) = if let (Some(x), Some(y)) = (rebind_x, rebind_y) {
        ...
        if let Some((bld_ent, resp_cost, owner_id, charge)) = resp_data {
            ...
            if respawn_here {
                ...
                find_random_public_resp(building_entities, ecs)
            }
        } else {
            find_random_public_resp(building_entities, ecs)
        }
...
fn find_random_public_resp(
    ...
    use rand::Rng;
    let public_resps: Vec<(i32, i32)> = building_entities
        .iter()
        .copied()
        .filter_map(|entity| {
            if ecs
        ...
        .modify_player(pid, |ecs, entity| {
            let (h, mh, px, py) = {
                let Some(s) = ecs.get::<crate::game::player::PlayerStats>(entity) else {
                    deferred.push(("OK", ok_message("СМЕРТЬ", "Состояние игрока недоступно.").1));
            ...
            deferred.push(("@L", health(new_h, mh).1));
            Some(if lethal {
                (true, Some((px, py)))
            } else {
                (false, Some((px, py)))
