...
fn secure_compare(a: &str, b: &str) -> bool {
    ...
    // Constant-time comparison of two fixed-length hashes
    let mut result = 0;
...
async fn auth_middleware(
    ...
    let authenticated = auth_header.is_some_and(|h| secure_compare(h, &token))
        || admin_header.is_some_and(|h| secure_compare(h, &token))
        || query_token.is_some_and(|h| secure_compare(h, &token));
...
async fn handle_auth(
    ...
    if secure_compare(&payload.token, &token) {
...
async fn handle_admin_commands() -> impl IntoResponse {
    Json(crate::admin::ADMIN_COMMANDS)
}
...
mod tests {
    ...
    fn test_secure_compare() {
        assert!(secure_compare("my-token", "my-token"));
        assert!(!secure_compare("my-token", "wrong-token"));
        assert!(!secure_compare("my-token", "my-token-longer"));
        assert!(!secure_compare("my-token-longer", "my-token"));
        assert!(!secure_compare("", "my-token"));
        assert!(!secure_compare("my-token", ""));
