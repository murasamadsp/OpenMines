    ...
    match event {
        "PROG" => {
            ...
            if let Some((prog_id, source)) = decoded {
                ...
                if running {
                    // Hydrate selected program before @P=1. The Unity client
                    // uses @P as the mode switch; if it arrives before #p, the
                    // editor window can reopen while the program is already
                    // running.
