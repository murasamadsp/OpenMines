...
fn send_initial_sync(
    ...
    // 17. UpdateProg (#p). Unity `@P 1` shows the programmator object; `#p`
    // hydrates the selected program and hides the editor view again. Keep it
    // after `@P/BH` during init so reconnecting to an offline-running program
    // does not leave the editor over gameplay.
    if let Some(program) = &player.selected_program {
        send_u_packet(
            tx,
            "#p",
...
mod tests {
    ...
    #[tokio::test]
    async fn init_running_selected_program_hydrates_without_opening_editor() {
